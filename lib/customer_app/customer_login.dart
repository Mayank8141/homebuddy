...
class customer_login_screen extends StatefulWidget {
  const customer_login_screen({super.key});

  @override
  State<customer_login_screen> createState() => _customer_login_screenState();
}
...
class _customer_login_screenState extends State<customer_login_screen> {
  ...
  Future<void> login()async{
    try{
      ...
      DocumentSnapshot userDoc=await _fireStore.collection('customer_detail').doc(userId).get();
      if(!userDoc.exists){
        throw Exception("user Recorde Not Found");

      }
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_)=>customer_home_screen()));
    }
    catch(e){
              ...
              children: [
                ...
                Center(child: InkWell(
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (_) => customer_register_screen()), // Change where needed
                    );
                  },
                  child: RichText(
                    text: TextSpan(
